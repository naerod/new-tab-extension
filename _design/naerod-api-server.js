...
function fetchLeetify() {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'api.cs-prod.leetify.com',
      path: `/api/profile/id/${STEAM_ID}`,
...
// caches
let leetifyCache = null, leetifyTime = 0;
...
async function getLeetify() {
  const now = Date.now();
  if (!leetifyCache || now - leetifyTime > LEETIFY_TTL) {
    const data = await fetchLeetify();
    ...
    leetifyCache.faceit = null;   // Faceit géré séparément via /api/faceit?nick=
    leetifyTime = now;
  }
  return leetifyCache;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url === '/api/leetify') {
      res.writeHead(200);
      return res.end(JSON.stringify(await getLeetify()));
    }
    if (url === '/api/faceit') {
      const qs = new URLSearchParams(req.url.split('?')[1] || '');
      if (!HOMELAB_KEY || qs.get('k') !== HOMELAB_KEY) { res.writeHead(401); return res.end('{"error":"unauthorized"}'); }
